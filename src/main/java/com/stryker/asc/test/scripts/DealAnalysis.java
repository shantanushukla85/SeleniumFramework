...
//			String price = lstRowData.get(commonLib.getColumnNumberFromList(headerRow, "Procedure_Price"));
			...
			if (Run_Status.equalsIgnoreCase("Yes")) {
				...
				try {
					...
					login.loginToSFDC(workBook, 3, 2);
					accountAndTerr.clickAppFromHeader("Accounts");
					account.clickOnAccountNameLink("Test_Automation_Finance_Account");
...
//					quote.clickOnAddToQuote();
	...
	@Test(priority = 1)
	public void dealAnalysis_Finance_231310_TC_02() throws Exception {
		String newOppName;
		boolean flag = false;
		String testCaseID = new Object() {
		}.getClass().getEnclosingMethod().getName();
...
//			String price = lstRowData.get(commonLib.getColumnNumberFromList(headerRow, "Procedure_Price"));
			...
			if (Run_Status.equalsIgnoreCase("Yes")) {
				...
				try {
					...
					commonLib.log(LogStatus.INFO,
							testCaseID + "Deal analysis - Finance: 164771 - ASC Profit Floors test case");

					login.loginToSFDC(workBook, 3, 3);
					accountAndTerr.clickAppFromHeader("Accounts");
					account.clickOnAccountNameLink("Test_Automation_Finance_Account");
					accountAndTerr.clickOnOpportunityNameUnderAccountPage("Test_Automation_Finance_Oppty");
